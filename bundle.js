...
var ScalarObservable = (function (_super) {
    __extends$14(ScalarObservable, _super);
...
var EmptyObservable = (function (_super) {
    __extends$15(EmptyObservable, _super);
...
var ArrayObservable = (function (_super) {
    __extends$13(ArrayObservable, _super);
...
var percentFormat = locale$3.format('.1%');
var intFormat = locale$3.format(',d');

var NaNColor = 'rgb(200,200,200)';

var scrutins = [{ label: 'Légis. 1', selector: 'legislatives-1' }, { label: 'Légis. 2', selector: 'legislatives-2' }, { label: 'Prés. 1', selector: 'presidentielle-1' }, { label: 'Prés. 2', selector: 'presidentielle-2' }];
...
var abstentionMetricParameters = {
  colors: scheme$21[5],
  label: 'Abs.'
};
...
var QuantileMetric = function (_Metric) {
  inherits(QuantileMetric, _Metric);

  function QuantileMetric(_ref2) {
    var colors = _ref2.colors;
    classCallCheck(this, QuantileMetric);
    return possibleConstructorReturn(this, (QuantileMetric.__proto__ || Object.getPrototypeOf(QuantileMetric)).call(this, {
      scale: quantile$$1().range(colors)
    }));
  }

  createClass(QuantileMetric, [{
    key: 'init',
    value: function init(data) {
      var _this2 = this;

      console.log(data.map(function (d) {
        return _this2._getValue(d);
      }));
      this.scale.domain(data.map(function (d) {
        return _this2._getValue(d);
      }));
    }
  }]);
...
var VoteMetric = function (_QuantileMetric) {
  inherits(VoteMetric, _QuantileMetric);

  function VoteMetric(_ref3) {
    var colors = _ref3.colors,
        nuances = _ref3.nuances,
        label = _ref3.label;
    classCallCheck(this, VoteMetric);

    var _this3 = possibleConstructorReturn(this, (VoteMetric.__proto__ || Object.getPrototypeOf(VoteMetric)).call(this, { colors: colors }));

    _this3.nuances = nuances;
    _this3.label = label;
    return _this3;
  }

  createClass(VoteMetric, [{
    key: '_getValue',
    value: function _getValue(d) {
      var _this4 = this;

      var candidat = d.candidats.find(function (c) {
        return _this4.nuances.includes(c.nuance);
      });
...
var votesMetrics = nuanceMetrics.map(function (descr) {
  return new VoteMetric(descr);
});

var abstentionMetric = Object.assign(new QuantileMetric({ colors: abstentionMetricParameters.colors }), {
  label: abstentionMetricParameters.label,
  _getValue: function _getValue(d) {
    return (d.inscrits - d.votants) / d.inscrits;
  }
});

var nuances = Object.keys(nuanceColors);

var premierScale = ordinal().domain(nuances).range(nuances.map(function (n) {
  return nuanceColors[n];
}));

var premierMetric = Object.assign(new Metric({ scale: premierScale, dotScale: null }), {
  label: '1er',
  _getValue: function _getValue(d) {
...
var metrics = [premierMetric, abstentionMetric].concat(toConsumableArray(votesMetrics));

var metricWrapper = function metricWrapper(_ref) {
  ...
  return {
    init: function init(data) {
      return metric.init(data.map(function (d) {
        return d.properties[scrutin];
      }).filter(function (d) {
        return d !== null;
      }));
    },
    getColor: function getColor(d) {
      return d.properties[scrutin] === null ? NaNColor : metric.getColor(d.properties[scrutin]);
    }
  };
};
...
var selector$1 = function () {
  ...
  var selector = function selector(elem) {
    elem.attr('class', 'selector');

    var scrutinSelector = elem.append('div').attr('class', 'scrutin');
    var metricSelector = elem.append('div').attr('class', 'metric');

    var scrutinOptions = scrutinSelector.selectAll('.option').data(scrutins);
    var scrutinOptionsEntering = scrutinOptions.enter().append('div').attr('class', 'option');

    scrutinOptionsEntering.append('input').attr('type', 'radio').property('checked', function (d, i) {
      return i === 0;
    }).attr('name', 'scrutin').attr('id', function (d, i) {
      return 'scrutin-' + i;
    }).on('click', function (d) {
      return scrutin$.next(d.selector);
    });

    scrutinOptionsEntering.append('label').attr('for', function (d, i) {
      return 'scrutin-' + i;
    }).text(function (d) {
      return d.label;
    });

    var metricOptions = metricSelector.selectAll('.option').data(metrics).enter().append('div').attr('class', 'option');

    metricOptions.append('input').attr('type', 'radio').property('checked', function (d, i) {
      return i === 0;
    }).attr('name', 'metric').attr('id', function (d, i) {
      return 'metric-' + i;
    }).on('click', function (d) {
      return rawMetric$.next(d);
    });

    metricOptions.append('label').attr('for', function (d, i) {
      return 'metric-' + i;
    }).text(function (d) {
      return d.label;
    });
  ...
  var scrutin$ = selector.scrutin$ = new ReplaySubject_2(1);
  var rawMetric$ = new ReplaySubject_2(1);
  selector.metric$ = Observable_2.combineLatest(scrutin$, rawMetric$).map(metricWrapper);
...
var width = 1000;
var height = 900;
var circosStrokeWidth = 0.5;
var departementsStrokeWidth = 1.5;

var map$7 = function (topology$$1, metric$) {

  var data = feature(topology$$1, topology$$1.objects.circonscriptions).features;

  function map(elem) {
    elem.attr('class', 'map');

    var svg = elem.append('svg').attr('width', width).attr('height', height);

    var zoomableGroup = svg.append('g');
    var circosGroup = zoomableGroup.append('g');

    var circos = circosGroup.selectAll('.circonscription').data(data);

    var path = geoPath().projection(null);

    circos = circos.enter().append('path').attr('class', 'circonscription').attr('d', path).attr('stroke', 'black').attr('stroke-width', circosStrokeWidth).on('click', function (d) {
      return circonscription$.next(d.properties);
    }).merge(circos);

    function changeFill(metric) {
      metric.init(data);
      var t = transition('circonscriptions').duration(1000);

      circos.transition(t).attr('fill', function (d) {
        return metric.getColor(d);
      });
    }

    var departements = zoomableGroup.append('path').datum(mesh(topology$$1, topology$$1.objects.circonscriptions, function (a, b) {
      return a === b || a.properties.departement !== b.properties.departement;
    })).attr('fill', 'none').attr('stroke', 'black').attr('stroke-width', departementsStrokeWidth).attr('d', path);
    ...
    var mapZoom = zoom().scaleExtent([1, 10]).translateExtent([[0, 0], [width, height]]).on("zoom", zoomed);

    svg.call(mapZoom);

    var t = transition().duration(250);

    elem.append('button').attr('class', 'reset-button fa fa-refresh').on('click', function () {
      return svg.transition(t).call(mapZoom.transform, identity$4);
    });
  }

  var circonscription$ = map.circonscription$ = new ReplaySubject_2(1);

  return map;
};

var ZA = "de la Guadeloupe";
var ZB = "de la Martinique";
var ZC = "de la Guyane";
var ZD = "de La Réunion";
var ZM = "de Mayotte";
var ZN = "de Nouvelle-Calédonie";
var ZS = "de Saint-Pierre-et-Miquelon";
var ZW = "de Wallis-et-Futuna";
var ZP = "de Polynésie Française";
var ZX = "de Saint-Barthélémy et Saint-Martin";
var ZZ = "des Français établis hors de France";
var designationDepartements = {
	ZA: ZA,
	ZB: ZB,
	ZC: ZC,
	ZD: ZD,
	...
	"02": "de l’Aisne",
	"03": "de l’Allier",
	"04": "des Alpes de Hautes-Provence",
	"05": "des Hautes-Alpes",
...
var details = function (scrutin$, circonscription$) {
  function details(elem) {
    elem.attr('class', 'details');

    var title = elem.append('h2').text('');

    var noDataMessage = elem.append('div').attr('class', 'hide').text('Pas de second tour dans cette circonscription.');

    // set up table
    var table = elem.append('table').attr('class', 'hide');
    var tbody = setUpTable(table);

    // set up scale for bars
    var x = linear().rangeRound([0, maxBarWidth]);

    Observable_2.combineLatest(scrutin$, circonscription$).subscribe(showDetails);

    function showDetails(_ref) {
      var _ref2 = slicedToArray(_ref, 2),
          scrutin = _ref2[0],
          circonscription = _ref2[1];
      ...
      var resultats = circonscription[scrutin];
      var data = formatData(resultats);

      x.domain([0, max(data, function (d) {
        return d.score;
      })]);

      var lignes = tbody.selectAll('tr').data(data, function (d) {
        return d.id;
      });

      // enter phase
      var lignesEnter = lignes.enter().append('tr').attr('class', function (d) {
        return d.nuance;
      });

      lignesEnter.append('th').text(function (d) {
        return d.nuance;
      }).attr('class', 'nuance').style('color', function (d) {
        return nuanceColors[d.nuance];
      });
      lignesEnter.append('td').text(function (d) {
        return d.label;
      }).attr('class', 'candidat');
      ...
      var lignesUpdate = lignesEnter.merge(lignes).order();

      lignesUpdate.select('.votes').text(function (d) {
        return d.votes;
      });
      lignesUpdate.select('.pourcentage').text(function (d) {
        return d.pourcentage;
      });
      lignesUpdate.select('.bar').style('width', function (d) {
        return x(d.score) + 'px';
...
function nomCirco(d) {
  var designation = designationDepartements[d.departement];
  var ordinal = d.circo === 1 ? 'ère' : 'ème';
...
function setUpTable(table) {
  ...
  [['nuance', ''], ['candidat', 'Candidat'], ['votes', 'Voix'], ['pourcentage', '%'], ['bar', '']].map(function (_ref3) {
    var _ref4 = slicedToArray(_ref3, 2),
        c = _ref4[0],
        l = _ref4[1];

    header.append('th').text(l).attr('class', c);
  });
...
function formatData(resultats) {
  ...
  var abstention = resultats.inscrits - resultats.votants;
  var abstentionInscrits = abstention / resultats.inscrits;
  var abstentionExprimes = abstention / resultats.exprimes;

  var data = resultats.candidats.map(function (c) {
    return {
...
var dashboard = function (topology) {
  function dashboard(root) {
    var s = selector$1();
    var m = map$7(topology, s.metric$);
    var d = details(s.scrutin$, m.circonscription$);

    root.append('div').call(s);

    var content = root.append('main');

    content.append('div').call(m);

    content.append('div').call(d);
  }
